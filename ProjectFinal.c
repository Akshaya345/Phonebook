...
int Verify_by_phoneNumber(long long int s1)  			  // Searching for Contact from Contact List By PhoneNumber
{ 
int f=0;
 pb s;
    FILE *op;
...
{	gotoxy(91,k++);
	...
	test=1;
   	...
   	if(Size(p[i].phone_number)!=10)                                    //Validation for  No.of Digits Entered 
   	{  gotoxy(91,k++);
	   printf("__Enter 10 digits Number !!__");
   	x=1;
	   }
 ...
 if(test!=1)                                                        //Validation for M or F
   ...
   x=1;
	...
	{x=1;
	 gotoxy(91,k++);
	 printf("__Enter 12 Digits Number!!__");
	}
	else
	{
...
{   pb s;
   ...
   op=fopen("Record.txt","r");
  	...
  	while(fread(&s, sizeof(pb), 1, op))   					//Reading the Data From the file 
    {
	  ...
	  printf("\nName :\t%s",s.name);
      printf("\nPhone Number :\t%lld",s.phone_number);
  	  printf("\nsex :\t%c",s.sex);
